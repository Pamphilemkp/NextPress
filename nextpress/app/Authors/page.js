...
const Page = () => {
    ...
    try {
      for (const authorId of authorIds) {
        const response = await fetch(
          `https://retrocket.github.io/retrocketeer-api/users/${authorId}.json`
        );
        const authorData = await response.json();
        combinedData.push(authorData);
      }

      setAuthorStore(combinedData);
    } catch (error) {
      console.error(`Error fetching data for author ${authorId}:`, error);
    }
  };
  ...
  return (
    <div className="bg-white p-8  rounded-lg shadow m-6">
      <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-white">All authors</h2>
      <p class="mb-12 text-xl font-normal text-center text-gray-500 dark:text-gray-200">
        Meet  all our  authors 
     </p>
      {authorStore.length > 0 ? (
